...
const GRADE = {
  ...
  formulas: [
    { title: "💧 小数乘除", items: [
      { name: "小数乘法", expr: "先按整数算，再数因数小数位数点小数点" },
      { name: "小数除法", expr: "除数变整数，被除数跟着移，小数点要对齐" }
    ] },
    { title: "❓ 方程", items: [
      { name: "等式性质", expr: "两边同时加减乘除同一个数（0除外），等式不变" }
    ] },
    { title: "📐 面积", items: [
      { name: "长方形", expr: "面积＝长×宽" },
      { name: "正方形", expr: "面积＝边长×边长" },
      { name: "平行四边形", expr: "面积＝底×高" },
      { name: "三角形", expr: "面积＝底×高÷2" },
      { name: "梯形", expr: "面积＝(上底＋下底)×高÷2" }
    ] },
    { title: "🔍 因数倍数", items: [
      { name: "2的倍数", expr: "个位是0、2、4、6、8" },
      { name: "5的倍数", expr: "个位是0或5" },
      { name: "3的倍数", expr: "各位数字和是3的倍数" },
      { name: "质数合数", expr: "1既不是质数也不是合数，2是最小的质数" }
    ] },
    { title: "🍰 分数", items: [
      { name: "约分", expr: "分子分母同时除以公因数，化到最简" },
      { name: "通分", expr: "化成同分母再比较、再加减" }
    ] },
    { title: "📦 体积", items: [
      { name: "长方体", expr: "体积＝长×宽×高" },
      { name: "正方体", expr: "体积＝棱长^3" },
      { name: "单位", expr: "1立方分米＝1000立方厘米，1升＝1000毫升＝1立方分米" }
    ] }
  ]
};
