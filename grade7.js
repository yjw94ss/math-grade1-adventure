...
const GRADE = {
  ...
  formulas: [
    { title: "🔢 有理数", items: [
      { name: "加法", expr: "同号相加取同号，异号相减取大号" },
      { name: "减法", expr: "减去一个数＝加上它的相反数" },
      { name: "乘除", expr: "同号得正，异号得负；除以一个数＝乘它的倒数" },
      { name: "乘方", expr: "a^n表示n个a相乘；负数偶次幂为正，奇次幂为负" },
      { name: "科学记数法", expr: "a×10^n（1≤|a|＜10）" }
    ] },
    { title: "🧩 整式", items: [
      { name: "合并同类项", expr: "系数相加，字母和指数不变" },
      { name: "去括号", expr: "括号前是减号，去掉括号全变号" }
    ] },
    { title: "❓ 一元一次方程", items: [
      { name: "步骤", expr: "去分母→去括号→移项（变号）→合并→系数化1" }
    ] },
    { title: "📏 平行线", items: [
      { name: "判定", expr: "同位角相等→平行；内错角相等→平行；同旁内角互补→平行" },
      { name: "性质", expr: "两直线平行→同位角/内错角相等，同旁内角互补" }
    ] },
    { title: "🌳 实数与坐标", items: [
      { name: "平方根", expr: "正数2个，0是0，负数没有；√a²＝|a|" },
      { name: "近似值", expr: "√2≈1.414，√3≈1.732，π≈3.14" },
      { name: "象限符号", expr: "一(＋,＋)，二(－,＋)，三(－,－)，四(＋,－)" }
    ] },
    { title: "🧮 方程组与不等式", items: [
      { name: "二元一次方程组", expr: "代入消元 或 加减消元，解完代回去检验" },
      { name: "不等式变向", expr: "乘除同一个负数，不等号方向改变" }
    ] }
  ]
};
